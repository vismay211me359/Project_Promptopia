...
export default function Form({type,post,setPost,submitting,handleSubmit}){
    return <section className="w-full max-w-full flex-start flex-col">
        ...
        <form onSubmit={handleSubmit} className="mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism">
            ...
            <label>
                <span className="font-satoshi font-semibold text-base text-gray-700">Tag{' '}<span className="fort-normal">(#product,#webdevelopment,#idea)</span></span>
            ...
            <div className="flex-end mx-3 mb-5 gap-4">
                ...
                <button type="submit" disabled={submitting} className="px-5 py-1 text-sm bg-primary-orange rounded-full text-white">{submitting ? `${type}ing...` : type}</button>
            </div>
        </form>
    </section>
}